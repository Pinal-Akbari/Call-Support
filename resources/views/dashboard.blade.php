...
@section('title', 'RootTech Telephony Portal')
    ...
    <div class="sidebar-user">
      <div class="sidebar-user-card">
        <div class="user-details" style="display:flex; align-items:center; gap:10px;">
          <div class="user-avatar" style="width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg, #6366f1, #a855f7); display:flex; align-items:center; justify-content:center; color:#fff; font-weight:bold; position:relative; flex-shrink:0;">
            <i class="fa-solid fa-user-gear"></i>
            <span id="sidebarStatusDot" class="status-indicator {{ strtolower($status) === 'offline' ? 'offline' : '' }}" style="position:absolute; bottom:0; right:0; width:10px; height:10px; border-radius:50%; border:2px solid var(--bg-card);"></span>
          </div>
          <div style="overflow:hidden;">
            <div class="user-title" style="font-weight:600; color:var(--text-bright); font-size:13px; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;">{{ $agentName }}</div>
            <div class="user-role" style="font-size:11px; color:var(--text-muted);">Ext: {{ $agentCode }} | {{ $queue }}</div>
          </div>
        </div>

        <a href="{{ route('logout') }}" class="btn btn-danger" style="width:100%; display:flex; align-items:center; justify-content:center; gap:8px; padding:8px 12px; font-size:13px; font-weight:600; border-radius:var(--radius-sm); text-decoration:none;">
  ...
  <main class="app-main">
    ...
    <header class="main-header">
      ...
      <div style="display: flex; align-items: center; gap: 14px;">
        <!-- Live Screen-Pop Listener Indicator -->
        ...
        <div class="header-status-box">
          ...
          <select id="statusSelect" class="form-input" style="padding: 4px 8px; font-size: 13px; border: none; background: transparent; width: auto; color: var(--text-main);">
            <option value="available" {{ strtolower($status) === 'available' ? 'selected' : '' }}>🟢 Available</option>
            <option value="break" {{ strtolower($status) === 'break' ? 'selected' : '' }}>🟡 On Break</option>
            <option value="wrapup" {{ strtolower($status) === 'wrapup' ? 'selected' : '' }}>🔵 Wrapup</option>
            <option value="offline" {{ strtolower($status) === 'offline' ? 'selected' : '' }}>🔴 Offline</option>
          </select>
        </div>
        <!-- Theme Toggle Button -->
        <button id="themeToggleBtn" class="btn btn-secondary" style="display:flex; align-items:center; gap:6px;">
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div>
            ...
            <div class="stat-value" id="statAgentStatus" style="text-transform: capitalize; color: #6ee7b7;">{{ $status }}</div>
          </div>
        </div>
        ...
        <div class="stat-card">
          ...
          <div>
            ...
            <div class="stat-value">{{ $agentCode }}</div>
          </div>
        </div>
      </div>
    ...
    <section id="module-call" class="module-section">
      ...
      <div class="glass-panel" style="margin-bottom: 24px;">
        <div style="display:flex; align-items:center; gap:12px; margin-bottom:8px;">
          <div style="width:40px; height:40px; border-radius:50%; background:linear-gradient(135deg,#6366f1,#a855f7); display:flex; align-items:center; justify-content:center;">
            <i class="fa-solid fa-phone-volume" style="color:#fff; font-size:16px;"></i>
          </div>
          <div>
            <h3 class="card-title" style="margin:0;">Click-To-Call Services</h3>
            <p style="font-size:12px; color:var(--text-muted); margin:2px 0 0;">Rings agent extension <strong style="color:#a5b4fc;">{{ $agentCode }}</strong> first, then connects to the mapped customer or maid securely via Universal BSNL DID masking (912612385555) for all calls.</p>
          </div>
        </div>
        ...
        <form id="moduleCallForm">
          <div class="form-grid">
            ...
            <div class="form-group">
              <label class="form-label"><i class="fa-solid fa-headset" style="color:#6366f1;"></i> Agent Extension (SIP)</label>
              <input type="text" id="callExtension" class="form-input" value="{{ $agentCode }}" readonly style="opacity:0.7;">
            </div>

            <div class="form-group">
              <label class="form-label"><i class="fa-solid fa-fingerprint" style="color:#06b6d4;"></i> Request ID <span style="font-size:10px; color:var(--text-dim);">(auto-generated)</span></label>
              <div style="display:flex; gap:8px;">
                <input type="text" id="callRequestId" class="form-input" value="REQ{{ time() }}" required>
                <button type="button" class="btn btn-secondary btn-sm" onclick="regenRequestId()" title="Generate new Request ID" style="flex-shrink:0;">
                  <i class="fa-solid fa-rotate"></i>
                </button>
              </div>
            </div>
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-top:16px;">
            <button type="button" id="btnCallCustomer" class="btn btn-primary active call-action-btn" onclick="triggerCallTarget('customer')">
              <i class="fa-solid fa-user-phone"></i>
              <div style="text-align:left;">
                <div style="font-weight:700;">Call Customer</div>
                <div style="font-size:11px; opacity:0.75;">POST ?r=call/customer</div>
              </div>
            </button>

            <button type="button" id="btnCallMaid" class="btn btn-secondary call-action-btn" onclick="triggerCallTarget('maid')">
              <i class="fa-solid fa-headset"></i>
              <div style="text-align:left;">
                <div style="font-weight:700;">Call Maid / Helper</div>
                <div style="font-size:11px; opacity:0.75;">POST ?r=call/maid</div>
              </div>
            </button>
          </div>
        </form>
      ...
      <div id="callResultCard" class="glass-panel" style="display:none; margin-bottom:24px;">
        <div class="card-header" style="margin-bottom:16px;">
          <div style="display:flex; align-items:center; gap:10px;">
            <div id="callLivePulse" class="call-pulse-ring"></div>
          ...
          <div style="display:flex; align-items:center; gap:10px;">
            ...
            <button type="button" class="btn btn-secondary btn-sm" id="btnPollStatus" onclick="manualPollStatus()" title="Refresh call status">
              <i class="fa-solid fa-rotate"></i>
        ...
        <!-- Full call data grid -->
        <div id="callDataGrid" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:12px; margin-bottom:16px;">
          <!-- filled by JS -->
        </div>

        <!-- Polling progress bar -->
        <div id="pollingProgressWrap" style="display:none; margin-bottom:12px;">
          <div style="display:flex; justify-content:space-between; font-size:11px; color:var(--text-dim); margin-bottom:4px;">
            <span><i class="fa-solid fa-satellite-dish"></i> Auto-polling call status...</span>
            <span id="pollCountLabel">0/10</span>
          </div>
          <div style="height:4px; background:rgba(99,102,241,0.2); border-radius:4px; overflow:hidden;">
            <div id="pollingProgressBar" style="height:100%; width:0%; background:linear-gradient(90deg,#6366f1,#a855f7); border-radius:4px; transition:width 0.5s ease;"></div>
          </div>
        </div>
      </div>
        ...
        <div class="card-header" style="margin-bottom:16px;">
          <div>
            <h3 class="card-title" style="margin-bottom:4px;">
              <i class="fa-solid fa-clock-rotate-left"></i>
              Call History <span id="callHistoryCount" class="badge badge-purple" style="margin-left:8px;">0 calls</span>
            </h3>
            <p style="font-size:12px; color:var(--text-muted); margin:0;">Per-session call log — all calls initiated this session</p>
          </div>
          <div style="display:flex; gap:10px; align-items:center;">
            <div style="position:relative;">
              <input type="text" id="callHistorySearch" class="form-input" placeholder="Search history..." style="padding-left:32px; font-size:13px; width:200px;" oninput="filterCallHistory()">
              <i class="fa-solid fa-magnifying-glass" style="position:absolute; left:10px; top:50%; transform:translateY(-50%); color:var(--text-dim); font-size:12px;"></i>
            </div>
            <button type="button" class="btn btn-danger btn-sm" onclick="clearCallHistory()">
            ...
            <tbody id="callHistoryTableBody">
              <tr id="callHistoryEmptyRow">
                <td colspan="10" style="text-align:center; padding:28px; color:var(--text-muted);">
                  ...
                  No calls initiated yet. Use the form above to start a call.
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <form id="moduleMapForm">
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                <input type="text" id="mMapMaskDid" class="form-input" value="{{ config('roottech.mask_did', '912612385555') }}" readonly style="opacity:0.85; background:rgba(99,102,241,0.06);" title="Universal Company BSNL DID" required>
              </div>
            </div>
            ...
            <div class="form-grid">
              <div class="form-group">
                ...
                <input type="text" id="mMapValidFrom" class="form-input" value="{{ date('Y-m-d H:i:s') }}" required>
              </div>
              <div class="form-group">
                <label class="form-label"><i class="fa-regular fa-calendar-xmark" style="color:#f59e0b;"></i> Valid Until</label>
                <input type="text" id="mMapValidUntil" class="form-input" value="{{ date('Y-m-d H:i:s', strtotime('+1 year')) }}" required>
              </div>
            </div>
      ...
      <div class="glass-panel">
        ...
        <form id="moduleRecFilterForm" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 14px; margin-bottom: 18px;">
          <div class="form-group" style="margin-bottom:0;">
            ...
            <input type="date" id="mRecFromDate" class="form-input" value="{{ date('Y-m-d') }}">
          </div>

          <div class="form-group" style="margin-bottom:0;">
            <label class="form-label">Date To</label>
            <input type="date" id="mRecToDate" class="form-input" value="{{ date('Y-m-d') }}">
          </div>
      ...
      <div class="glass-panel">
        ...
        <form id="moduleReportsFilterForm" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px, 1fr)); gap:12px; margin-bottom:20px; background:var(--bg-user-box); padding:16px; border-radius:var(--radius-md); border:1px solid var(--border-glass);">
          ...
          <div class="form-group" style="margin-bottom:0;">
            ...
            <input type="date" id="mRepToDate" class="form-input" value="{{ date('Y-m-d') }}">
          </div>
...
<!-- Floating Incoming Call Screen-Pop Alert Modal -->
<div id="incomingCallPopupModal" class="glass-panel" style="display:none; position:fixed; top:24px; right:24px; z-index:2000; width:380px; box-shadow:var(--shadow-modal); border:2px solid var(--primary); background:var(--modal-bg);">
  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid var(--border-glass); padding-bottom:8px;">
    <div style="display:flex; align-items:center; gap:8px;">
      <div style="width:32px; height:32px; border-radius:50%; background:rgba(16,185,129,0.15); color:#10b981; display:flex; align-items:center; justify-content:center; font-size:14px;">
        <i class="fa-solid fa-phone-volume"></i>
      </div>
      <div>
        <h4 style="margin:0; font-size:14px; color:var(--text-bright);">Incoming Call Event</h4>
        <span style="font-size:11px; color:var(--text-dim);" id="popupEventTime">Just now</span>
      </div>
    </div>
    <button type="button" class="btn btn-secondary btn-sm" onclick="dismissCallPopup()" style="padding:2px 6px;"><i class="fa-solid fa-xmark"></i></button>
  </div>

  <div style="background:var(--bg-user-box); padding:12px; border-radius:var(--radius-sm); margin-bottom:12px; font-size:13px;">
    <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
      <span style="color:var(--text-muted);">Caller Number:</span>
      <strong id="popupCallerNumber" style="color:#6ee7b7; font-family:monospace;">—</strong>
    </div>
    <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
      <span style="color:var(--text-muted);">Booking ID:</span>
      <strong id="popupBookingId" style="color:#a5b4fc;">—</strong>
    </div>
    <div style="display:flex; justify-content:space-between;">
      <span style="color:var(--text-muted);">Queue / Event:</span>
      <span id="popupQueue" class="badge badge-purple">root-support</span>
    </div>
  </div>

  <div style="display:flex; gap:8px;">
    <button type="button" class="btn btn-primary btn-sm" onclick="acknowledgeCallPopup()" style="flex:1;">
      <i class="fa-solid fa-check"></i>
      <span>Acknowledge</span>
    </button>
    <button type="button" class="btn btn-secondary btn-sm" onclick="dismissCallPopup()">
      <span>Dismiss</span>
    </button>
  </div>
</div>
